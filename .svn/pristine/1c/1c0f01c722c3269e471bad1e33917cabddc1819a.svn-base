...
@interface ConsonantsCell ()<UICollectionViewDelegateFlowLayout, UICollectionViewDelegate, UICollectionViewDataSource>
...
- (void)setRecord:(Record *)record
{
    ...
    [attrString addAttribute:NSFontAttributeName value:firstFont range:NSMakeRange(0, length-1)];
    UIFont *baseFont = [UIFont fontWithName:@"STHeitiSC-Light" size:56];
    [attrString addAttribute:NSFontAttributeName value:baseFont range:NSMakeRange(length-1, 1)];
    UIColor *color = [UIColor colorWithWhite:0 alpha:0.05];
    [attrString addAttribute:NSForegroundColorAttributeName value:color range:NSMakeRange(length-1, 1)];
    ...
    if (images)
    {
        ...
        {
            NSString *path = [Record pathOfPicutre:name bundle:@"ConsonantsImage"];
    ...
    self.audioControlView.sourceAudio = [Record pathOfAudio:record.picName type:@"mp3" bundle:@"Consonants"];
}
