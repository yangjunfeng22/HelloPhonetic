...
@synthesize animate=_animate, color=_color, progressBackgroundColor = _progressBackgroundColor;
...
- (void)drawRightAlignedLabelInRect:(CGRect)rect {
    if (rect.size.width > 40) {
        ...
        label.font = self.textFont;//[UIFont boldSystemFontOfSize:17];
        //UIColor *baseLabelColor = [self.color isLighterColor] ? [UIColor blackColor] : [UIColor whiteColor];
        label.textColor = self.textColor;//[baseLabelColor colorWithAlphaComponent:0.6];
...
- (UIColor *)progressBackgroundColor
{
    ...
    {
        return [UIColor colorWithRed:218/255.0 green:219/255.0 blue:221/255.0 alpha:1.0];
    }
