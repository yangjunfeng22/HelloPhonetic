...
- (void)drawRectangleInContext:(CGContextRef)context StartX:(CGFloat)aStartX EndX:(CGFloat)aEndX Fill:(BOOL)aFill
{
    ...
    CGFloat cornerW = CGRectGetWidth(rect) > self.bounds.size.height ? self.bounds.size.height*0.5:CGRectGetWidth(rect)*0.5;
    CGMutablePathRef trackPath = CGPathCreateMutable();
    CGPathAddRoundedRect(trackPath, NULL, rect, cornerW, self.bounds.size.height*0.5);
...
- (void)drawLayer:(CALayer *)layer inContext:(CGContextRef)ctx
{
    ...
    NSString *text = [NSString stringWithFormat:@"%.0f%%", self.current*100];
    CGRect rectToDrawIn = CGRectMake(0, 0, rect.size.width * self.current, rect.size.height);
    CGRect insetRect = CGRectInset(rectToDrawIn, self.current > 0.03 ? 0.5 : -0.5, 0.5);
    
    CGRect frame = CGRectMake(rectToDrawIn.size.width-40, 5, insetRect.size.width-12, insetRect.size.height);
    
    [text drawInRect:frame withAttributes:@{NSFontAttributeName:[UIFont boldSystemFontOfSize:12], NSForegroundColorAttributeName:[UIColor whiteColor]}];
...
- (void)drawRightAlignedLabelInRect:(CGRect)rect
{
    if (rect.size.width > 40) {
        ...
        label.text = [NSString stringWithFormat:@"%.0f%%", self.current*100];
