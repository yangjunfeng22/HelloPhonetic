...
@property (nonatomic, readwrite) CGFloat playDuration;
...
- (void)commonSetup
{
    if (self.currentButtonStyle == audioButtonRoundedStyle)
    {
        ...
        self.bckgLayer.cornerRadius = self.bckgLayer.bounds.size.height*0.5;
...
- (void)playAudio:(NSString *)audio completion:(void (^)(BOOL finished, NSError *error))completion
{
    ...
    // 0相当于最低音,最低音的话就是用系统的音量。
    audioPlayer.volume = 0;
    audioPlayer.loops = self.loops;
    
    CGFloat duration = audioPlayer.duration*(audioPlayer.loops+1);
    _playDuration = duration;
    if (_currentButtonStyle == audioButtonRoundedStyle){
        [self.circleView setAnimatedStrokeEnd:1 duration:duration];
    }
...
- (CircleView *)circleView
{
    if (!_circleView)
    {
        ...
        _circleView.strokeColor = kColorCircle;
...
-(UIImageView *)voiceAnimationImgView
{
    if (!_voiceAnimationImgView)
    {
        ...
        _voiceAnimationImgView.animationImages = [NSArray arrayWithObjects:ImageNamed(@"ico_audioPlay_0"), ImageNamed(@"ico_audioPlay_1"), ImageNamed(@"ico_audioPlay_2"), ImageNamed(@"ico_audioPlay_3"), nil];
