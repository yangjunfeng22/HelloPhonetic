...
- (void)drawInContext:(CGContextRef)ctx
{
    ...
    {
        case 0:
//            [self drawLightToneInContext:ctx];
//            break;
        case 1:
        case 2:
        case 3:
        ...
        case 44:
        case 40:
            [self drawToneBackgroundColorInContext:ctx];
            break;
        default:
...
- (void)drawToneBackgroundColorInContext:(CGContextRef)ctx
{
    CGRect rect = self.bounds;
    
    CGFloat totalWidth = rect.size.width - 4;
    
    CGFloat progress = MIN(self.progress, 1.0f - FLT_EPSILON);
    
    CGRect frame = CGRectMake(totalWidth*self.oProgress+2, 3, totalWidth*(progress-self.oProgress), rect.size.height-6);
...
// 轻声(0声)
- (void)drawLightToneInContext:(CGContextRef)ctx
{
    CGRect rect = self.bounds;
    CGFloat totalWidth = kToneHeight;
    ...
    CGFloat cornerW = CGRectGetWidth(frame) > self.toneHeight ? self.toneHeight*0.5:CGRectGetWidth(frame)*0.5;
    CGContextSetFillColorWithColor(ctx, self.toneTintColor.CGColor);
    CGContextSetAlpha(ctx, self.progress);
    CGMutablePathRef tonePath = CGPathCreateMutable();
    CGPathAddRoundedRect(tonePath, NULL, bFrame, cornerW, self.toneHeight*0.5);
    CGPathCloseSubpath(tonePath);
    CGContextAddPath(ctx, tonePath);
    CGContextFillPath(ctx);
...
- (void)drawLevelToneInContext:(CGContextRef)ctx
{
    ...
    CGFloat totalWidth = rect.size.width * 0.8;
    CGRect bFrame = CGRectMake(rect.size.width * 0.1, 8, totalWidth, self.toneHeight);
    CGContextSetFillColorWithColor(ctx, self.trackTintColor.CGColor);
    CGMutablePathRef trackPath = CGPathCreateMutable();
    CGPathAddRoundedRect(trackPath, NULL, bFrame, self.toneHeight*0.5, self.toneHeight*0.5);
    CGPathCloseSubpath(trackPath);
    ...
    CGRect frame = CGRectMake(totalWidth*self.oProgress+rect.size.width * 0.1, 8, totalWidth*(progress-self.oProgress), self.toneHeight);
    CGFloat cornerW = CGRectGetWidth(frame) > self.toneHeight ? self.toneHeight*0.5:CGRectGetWidth(frame)*0.5;
...
- (void)drawThirdToneInContext:(CGContextRef)ctx
{
    ...
    // 音调条的背景
    // 4声的方向
    CGRect bf1 = CGRectMake(-totalWidth*0.5, -self.toneHeight*0.5, s+cornerFactor, self.toneHeight);
    CGRect bf2 = CGRectMake(totalWidth*0.5-s-cornerFactor, -self.toneHeight*0.5, s+cornerFactor, self.toneHeight);
    ...
    {
        // 上面那一步画完之后，要将向下的条直接加上。
        CGRect frame3 = CGRectMake(-totalWidth*0.5, -self.toneHeight*0.5, s+cornerFactor, self.toneHeight);
        CGAffineTransform tm1 = CGAffineTransformMakeRotation(-M_PI_4*0.5);
        CGPathAddRoundedRect(tonePath, &tm1, frame3, self.toneHeight*0.5, self.toneHeight*0.5);
        
        // 画向上的图
        CGRect frame2 = CGRectMake(totalWidth*0.5-s-cornerFactor + s*self.oProgress, -self.toneHeight*0.5, s * (progress-self.oProgress)+cornerFactor, self.toneHeight);
...
+ (void)initialize
{
    if (self == [HSToneView class])
    {
        ...
        [toneViewAppearance setTrackTintColor:[UIColor colorWithRed:0.9 green:0.9 blue:0.9 alpha:1.0]/*[[UIColor whiteColor] colorWithAlphaComponent:0.3]*/];
