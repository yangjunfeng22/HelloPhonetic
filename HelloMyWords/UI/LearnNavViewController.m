...
//    [self.checkPointScrollView setContentSize:contentSize];
    ...
    [self startImport:toneKnowledge type:DataImportTypeToneKnowledge completion:^(BOOL finished) {
...
- (void)refreshTitle
{
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        NSInteger page = floor((self.checkPointScrollView.contentOffset.x - pageWidth/2) / pageWidth) + 1;
        dispatch_async(dispatch_get_main_queue(), ^{
//            UIImage *imgTitle = [UIImage imageNamed:@"pinyin"];
            if (page >= 1) {
                self.lblTitle.text = @"拼  音^pīn yīn";
//                imgTitle = [UIImage imageNamed:@"pinyin"];
                
            }else{
                self.lblTitle.text = @"声  调^shēng diào";
//                imgTitle = [UIImage imageNamed:@"shengdiao"];
            }
            
            //[self.imgvTitle showClipImageWithImage:imgTitle];
//            self.imgvTitle.image = imgTitle;
        });
    });
}
